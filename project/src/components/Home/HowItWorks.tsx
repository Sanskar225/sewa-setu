      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {steps.map((step, index) => (
            <div key={step.id} className="relative">
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-8 left-full w-8 h-0.5 bg-white transform -translate-x-4"></div>
              )}

              {/* Step Number */}
              <div className="absolute -top-2 -left-2 w-6 h-6 bg-white text-black rounded-full flex items-center justify-center text-sm font-bold">
                {step.id}
              </div>
            </div>
          ))}
        ...
        <div className="text-center mt-16">
          <button
            ...
            className="bg-white text-black px-8 py-3 rounded-lg hover:bg-gray-800 transition-colors font-medium mr-4"
          >
            Get Started
          </button>
          <button className="border border-gray-300 text-white px-8 py-3 rounded-lg hover:bg-white transition-colors font-medium">
            Learn More
          </button>
        </div>
      </div>
