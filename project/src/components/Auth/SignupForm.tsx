...
import { BookOpen, Mail, Lock, Eye, EyeOff, User, Shield, ArrowRight, Star, CheckCircle, Phone } from 'lucide-react';
...
const SignupForm: React.FC = () => {
  ...
  const handleRoleChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setFormData(prev => ({ ...prev, role: value as 'USER' | 'PROVIDER' | 'ADMIN'}));
    if (value === 'ADMIN') {
      try {
  ...
  <div className="relative">
    ...
    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
      <div className="bg-green-600 w-8 h-8 rounded-lg flex items-center justify-center">
        <Lock className="h-4 w-4 text-white" /> {/* You can replace this with a Phone icon */}
      </div>
    </div>
    ...
    <input
      ...
      onChange={handleInputChange} // ✅ Must include this
    ...
    >
      <div className="bg-slate-600 w-8 h-8 rounded-lg flex items-center justify-center">
        <Phone className="h-4 w-4 text-white" /> {/* Optional: Use lucide-react Phone icon */}
      </div>
    </button>
  </div>
</div>
